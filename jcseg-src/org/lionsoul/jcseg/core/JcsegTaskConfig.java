...
//		this(null);
	...
	public JcsegTaskConfig( String proFile ) {
		...
		try {
			resetFromPropertyFileName(proFile);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	public JcsegTaskConfig( InputStream proFile ) {
	...
	public void resetFromPropertyFileName( String proFile ) throws IOException {
		File pro_file = new File(proFile);
		if (proFile == null) resetFromPropertyFile(null);
		else {
			resetFromPropertyFile(new FileInputStream(pro_file));
		}
	}
	...
	@SuppressLint("NewApi")
	public void resetFromPropertyFile( InputStream proFile ) throws IOException {
